...
Description: The Codeaft website can be set up and operated on localhost with the aid of this script.
"""
...
def install_packages():
    if ubuntu:
        ...
        if not githubactions:
            if codeaft not in os.getcwd():
                os.system("git clone https://github.com/codeaft/codeaft.github.io.git")
                os.chdir(cwd + codeaft + "/")
        install_chrome()
...
def start_setup():
    ...
    if connected_to_internet():
        if freedisk >= 2:
            if full:
                if os.path.split(os.getcwd())[-1] == codeaft:
